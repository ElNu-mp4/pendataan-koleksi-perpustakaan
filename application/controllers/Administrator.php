...
class Administrator extends CI_Controller {
    ...
    public function debug_pendataan() {
        // Only allow admin
        if ($this->session->level !== 'admin') {
            redirect('administrator/pendataan');
        }
        
        include(APPPATH . 'views/administrator/debug_pendataan.php');
    }
    ...
    // Controller Modul Pendataan

    function config_pendataan(){
        cek_session_akses('pendataan',$this->session->id_session);
        
        if (isset($_POST['submit'])){
            $data = array(
                'tahun_aktif' => $this->input->post('tahun_aktif'),
                'status_pendataan' => $this->input->post('status_pendataan'),
                'pesan_nonaktif' => $this->input->post('pesan_nonaktif')
            );
            $where = array('id' => 1);
            $this->model_app->update('config_pendataan', $data, $where);
            
            $this->session->set_flashdata('success', 'Konfigurasi pendataan berhasil diperbarui');
            redirect('administrator/pendataan');
        } else {
            $proses = $this->model_app->edit('config_pendataan', array('id' => 1))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_pendataan/view_config_pendataan',$data);
        }
    }
    ...
    function pendataan() {
        ...
        if (!$config) {
            ...
            $default_config = [
                'tahun_aktif' => date('Y'),
                'status_pendataan' => 'aktif',
                'pesan_nonaktif' => 'Pendataan saat ini sedang tidak dibuka. Silakan hubungi administrator untuk informasi lebih lanjut.'
            ];
            $this->db->insert('config_pendataan', $default_config);
            $config = $default_config;
            $config['id'] = $this->db->insert_id();
        }
        
        // Filter
        $tahun = $this->input->get('tahun') ?: ($config['tahun_aktif'] ?: date('Y'));
        $nomor_pokok = $this->input->get('nomor_pokok') ?: '';
        ...
        // Ambil nama perpustakaan untuk setiap record
        ...
        $this->template->load('administrator/template', 'administrator/mod_pendataan/view_pendataan', $data);
    }

    function tambah_pendataan() {
        cek_session_akses('pendataan', $this->session->id_session);
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($jawaban_post)) {
                redirect('administrator/tambah_pendataan');
            }
            
            $this->db->trans_begin();
            
            $this->db->insert('identitas_input_koleksi', array(
                'nomor_pokok' => '-',
                'tahun_data' => $this->input->post('tahun_data') ?: date('Y'),
            ...
            foreach ($jawaban_post as $id_pertanyaan => $jawaban) {
                ...
                $this->db->insert('jawaban_koleksi', array(
                    'id_koleksi' => $id_koleksi,
                    'id_pertanyaan' => (int)$id_pertanyaan,
                    'jawaban' => trim($jawaban)
                ));
                
                // Update nomor_pokok dari id_pertanyaan = 1
                if ($id_pertanyaan == 1) {
                    $this->db->update(
                        'identitas_input_koleksi',
                        array('nomor_pokok' => trim($jawaban)),
                        array('id_koleksi' => $id_koleksi)
                    );
                }
            }
            ...
            // Load pertanyaan
            $this->db->from('pertanyaan_koleksi');
            ...
            foreach ($query->result_array() as $row) {
                ...
                if (in_array($row['tipe_jawaban'], array('select', 'radio', 'checkbox'))) {
                    $row['opsi'] = $this->db
                        ->where('id_pertanyaan', $row['id_pertanyaan'])
                        ->where('aktif', 1)
                        ->order_by('urutan', 'ASC')
                        ->get('opsi_pertanyaan_koleksi')
                        ->result_array();
                }
                
                $pertanyaan[$section][] = $row;
            }
            
            $data = array(
                'pertanyaan' => $pertanyaan,
                'section_names' => $this->get_pendataan_section_names()
            );
            
            $this->template->load('administrator/template', 'administrator/mod_pendataan/view_pendataan_tambah', $data);
        }
    }

    function edit_pendataan() {
        cek_session_akses('pendataan', $this->session->id_session);
        $id = $this->uri->segment(3);
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($jawaban_post)) {
                redirect('administrator/edit_pendataan/' . $id);
            }
            ...
            $this->db->update(
                'identitas_input_koleksi',
                array('tahun_data' => $this->input->post('tahun_data') ?: date('Y')),
                array('id_koleksi' => $id)
            );
            
            // Hapus jawaban lama
            $this->db->where('id_koleksi', $id);
            $this->db->delete('jawaban_koleksi');
            ...
            $this->db->select('id_pertanyaan, jawaban');
            $this->db->from('jawaban_koleksi');
            $this->db->where('id_koleksi', $id);
            $jawaban_query = $this->db->get()->result_array();
            
            $jawaban_existing = array();
            foreach ($jawaban_query as $j) {
                $jawaban_existing[$j['id_pertanyaan']] = $j['jawaban'];
            }
            ...
            $data = array(
                'rows' => $identitas,
                'jawaban_existing' => $jawaban_existing,
                'pertanyaan' => $pertanyaan,
                'section_names' => $this->get_pendataan_section_names()
            );
            
            $this->template->load('administrator/template', 'administrator/mod_pendataan/view_pendataan_edit', $data);
        }
    }

    function delete_pendataan() {
        cek_session_akses('pendataan', $this->session->id_session);
        
        // Load cache driver for this function
        $this->load->driver('cache', ['adapter' => 'file', 'backup' => 'file']);
        
        // Get ID dari URI segment
        ...
        try {
            ...
            // Hapus identitas
            $this->db->where('id_koleksi', $id_koleksi);
            $this->db->delete('identitas_input_koleksi');
            ...
            // Hapus cache detail spesifik
            $this->cache->delete('pendataan/detail_koleksi_' . $id_koleksi);
    ...
    function detail_pendataan() {
        ...
        $identitas['nama_perpustakaan'] = $nama_perpustakaan ? $nama_perpustakaan['jawaban'] : $identitas['nomor_pokok'];
        ...
        $this->template->load('administrator/template', 'administrator/mod_pendataan/view_pendataan_detail', $data);
    }

    public function export_pendataan() {
        cek_session_akses('pendataan', $this->session->id_session);
        
        @set_time_limit(600);
        @ini_set('memory_limit', '512M');
        
        // Disable error display BEFORE any output
        @ini_set('display_errors', 0);
        @error_reporting(0);
        
        // Get filters with null safety
        $tahun = $this->input->get('tahun');
        if (!$tahun || !preg_match('/^(19|20)\d{2}$/', $tahun)) {
            $tahun = date('Y');
        }
        
        $nomor_pokok = $this->input->get('nomor_pokok');
        if ($nomor_pokok) {
            $nomor_pokok = preg_replace('/[^a-zA-Z0-9\-\/\s]/', '', $nomor_pokok);
        }
        
        // Build query - NO COUNT FIRST
        $this->db->select('i.*');
        $this->db->from('identitas_input_koleksi i');
        $this->db->where('i.tahun_data', $tahun);
        if (!empty($nomor_pokok)) {
        ...
        // Get first to check if data exists
        $check = $this->db->limit(1)->get()->row_array();
        ...
        $filename = 'rekap_pendataan_' . $tahun . '_' . date('YmdHis') . '.csv';
        ...
        // Header row
        $header = ['ID', 'Nomor Pokok', 'Nama Perpustakaan', 'Tahun', 'Tanggal', 'IP Address'];
        ...
        while ($has_more) {
            ...
            foreach ($rekap_chunk as $row) {
                $row_data = [
                    ...
                    $this->_sanitize_csv_value($row['nama_perpustakaan'] ?? ''),
                    $row['tahun_data'] ?? '',
                    $row['tanggal_submit'] ?? '',
                    $row['ip_address'] ?? ''
                ];
            ...
            if (count($rekap_chunk) < $chunk_size) {
                $has_more = false;
            }
    ...
    // Helper function untuk sanitize CSV values with null safety
    ...
    // Helper function for section names
    private function get_pendataan_section_names() {
        return array(
            1  => 'KELEMBAGAAN',
            2  => 'DATA PENGISI KUESIONER',
            3  => 'JUMLAH KOLEKSI',
            4  => 'PENAMBAHAN KOLEKSI',
            5  => 'ANGGARAN',
            6  => 'PEMANFAATAN KOLEKSI TERCETAK',
            7  => 'PEMANFAATAN KOLEKSI DIGITAL',
            8  => 'JUMLAH PENGUNJUNG',
            9  => 'AKSES PERANGKAT TEKNOLOGI',
            10 => 'ANGGOTA PERPUSTAKAAN',
